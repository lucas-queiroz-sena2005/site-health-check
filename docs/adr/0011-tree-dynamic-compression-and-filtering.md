...
### 2. Filter Immunity
To prevent a node that is acting as the anchor for a filter (e.g., a CIDR node where the user clicked "2 Ghosts") from compressing and losing its ID/identity, we introduced **Filter Immunity**. 
* Any node that possesses an *explicitly applied filter* (`isExplicitFilter`) is strictly prohibited from participating in compression. It will remain structurally distinct as the visible root of the filtered view.

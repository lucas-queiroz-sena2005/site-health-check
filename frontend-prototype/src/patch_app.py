...
old_handle_click = '''  const handleStatusClick = (nodeId, statusKey) => {
...
new_handle_click = '''  const handleStatusClick = (nodeId, statusKey) => {
    const newFilters = { search: '', statuses: [statusKey] };
    setExplicitFilters(prev => ({
      ...prev,
      [nodeId]: newFilters
...
content = re.sub(r'const \[activeViewId, setActiveViewId\] = useState\(\'view-default\'\);', 'const [activeViewId, setActiveViewId] = useState(\'view-default\');\n  const [isSavingView, setIsSavingView] = useState(false);\n  const [newViewName, setNewViewName] = useState(\'\');', content)
content = re.sub(r'sortBy:\s*\'none\'', 'tableSortBy: \'none\'', content)
content = re.sub(r'sortBy:\s*\'status\'', 'tableSortBy: \'status\'', content)
content = re.sub(r'sortDir:\s*\'asc\'', 'tableSortDir: \'asc\'', content)
content = re.sub(r'sortDir:\s*\'desc\'', 'tableSortDir: \'desc\'', content)

old_handle_select = '''  const handleSelectSavedView = (viewId) => {
...
new_handle_select = '''  const handleSelectSavedView = (viewId) => {
    setActiveViewId(viewId);
    if (viewId === 'custom-new') {
...
old_filtered_memo = '''  const filteredData = useMemo(() => {
...
new_filtered_memo = '''  const filteredData = useMemo(() => {
    const parentFilter = { search: globalSearch, statuses: globalStatuses };
    const filtered = filterTree(treeData, parentFilter, explicitFilters, ghostWindowHours);
...
content = content.replace('sortBy={sortBy}', 'sortBy={tableSortBy}')
content = content.replace('sortDir={sortDir}', 'sortDir={tableSortDir}')
content = content.replace(
'''                <select
                  value={sortBy}
                  onChange={(e) => {
...
content = content.replace('w-[120px]', 'min-w-[110px]', 1) # TLS
content = content.replace('w-[120px]', 'min-w-[90px]', 1) # Latency
...
with open('/home/crow/Projects/site-health-check/frontend/src/App.jsx', 'w') as f:
    f.write(content)
